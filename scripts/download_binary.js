...
async function getAsset() {
  ...
  headers.append("Authorization", `token ${process.env.TOKEN ?? "TOKEN is not defined"}`);

  const response = await fetch(process.env.GH_TAGS ?? "GH_TAGS is not defined", { headers });
  ...
  const asset = json.assets.find((asset) => asset.name.match(/unknown-linux-gnu\.zip$/g) !== null);
...
async function downloadFile(asset) {
  ...
  const desiredPath = path.join(__dirname, "..", "api_blog.zip");
